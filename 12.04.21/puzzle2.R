...
draw_and_check_all <- function(boards, number, method = "first"){
  updated_boards <- boards %>% map(draw_and_check, number = number)
  check <- lapply(updated_boards, bingo_check)
  if (method == "first" & any(unlist(check))){
    winning_number <<- number
    done(updated_boards)
  }
  else if (method == "last" & sum(unlist(check)) == 99){
    done(updated_boards)
  }
  else{
    updated_boards
...
final_boards <- numbers %>% reduce(draw_and_check_all, .init = boards, 
                                   method = "last")
board <- numbers %>% reduce(draw_and_check_all, .init = id_last(final_boards))
calc_score(unlist(board), winning_number)