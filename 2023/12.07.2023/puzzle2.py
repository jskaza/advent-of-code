...
def get_rank_info(hand: str, bid: int) -> list:
    dist = Counter(hand)
    if len(dist) != 1 and "J" in dist:
        ct = dist["J"]
        del dist["J"]
        dist[dist.most_common(1)[0][0]] += ct
    dist = dist.most_common(5)
    if len(dist) == 1:
        kind = 7
    elif dist[0][1] == 4:
        kind = 6
        ...
        kind = 1
    return [kind]+[card_numeric(c) for c in hand] + [bid]
