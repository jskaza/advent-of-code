...
def find_smudge_reflection(pattern):
    for row in range(pattern.shape[0]-1):
        ...
        if np.sum(pattern[row+1-reflection:row+1,] != np.flip(pattern[row+1:row+1+reflection,:], axis=0)) == 1:
            return 100*(row+1)
    for col in range(pattern.shape[1]-1):
        reflection = min(col+1, pattern.shape[1] - col - 1)
        if np.sum(pattern[:,col+1-reflection:col+1] != np.flip(pattern[:,col+1:col+1+reflection], axis=1)) == 1:
            return col+1

print(sum([find_smudge_reflection(pattern) for pattern in patterns]))
